...
mainGroupbox:AddSlider("extenderSize", { Text = "Size", Min = 2, Max = 1000, Default = 10, Rounding = 1 }):OnChanged(updatePlayers)
...
local WatermarkConnection = game:GetService('RunService').RenderStepped:Connect(function()
    ...
    Library:SetWatermark(('bbot.fun | Private | Tycoon | %s fps | %s ms'):format(
        math.floor(FPS),
...
updatePlayers()
Library:Notify("welcome to bbot.fun")
Library:Notify("be aware of bugs or some options dont working")
Library:Notify("because wave cannot change some client values")
